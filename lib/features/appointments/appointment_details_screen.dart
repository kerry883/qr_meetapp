...
class AppointmentDetailsScreen extends StatelessWidget {
  ...
  const AppointmentDetailsScreen({super.key, required this.id, this.appointment});

  @override
  Widget build(BuildContext context) {
    final state = context.watch<AppointmentState>();
    final appt = appointment ?? state.selectedAppointment;

    return Scaffold(
      appBar: AppBar(title: const Text('Appointment Details')),
      body: FutureBuilder(
        future: appt == null ? context.read<AppointmentState>().loadById(id) : null,
        builder: (context, snapshot) {
          final current = appointment ?? context.watch<AppointmentState>().selectedAppointment;
          if (current == null) {
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                Text(current.title, style: AppStyles.heading3),
                const SizedBox(height: 8),
                Text(current.description, style: AppStyles.bodyMedium.copyWith(color: AppColors.grey700)),
                ...
                Row(children: [
                  ...
                  const SizedBox(width: 6),
                  Text(current.formattedTime),
                ]),
                const SizedBox(height: 16),
                Text('Location: ${current.location}', style: AppStyles.bodyMedium),
                const Spacer(),
                Row(
                  children: [
                    Expanded(
                      child: OutlinedButton(
                        onPressed: () {
                          // TODO: open reschedule flow/form
